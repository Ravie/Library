...
public class Library implements EntryPoint, DialogBoxAction {
    ...
    public void applyChanges(int action) {
        if(action == -1) {
            service.addBook(selectedPage, booksNum, sorting, new Book(dialogBox.getAuthor(), dialogBox.getTitle(), dialogBox.getPageNum(), dialogBox.getPublishingYear(), new Date().getYear() + 1900, new Date().getMonth() + 1, new Date().getDate()), new MethodCallback<BookPagination>() {
                @Override
                public void onFailure(Method method, Throwable throwable) {
